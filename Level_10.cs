...
{
    public partial class Level_10 : Form
    {
        Graphics g;
        const int n = 5;
        ...
        Image yel = new Bitmap("oOWEHQ-hHi81.jpg");
        Image bl = new Bitmap("f1f8rTxyHNM1.png");
        ...
        private void draw_str(Button[,] btn, int[,] a, int c)
        {
            for (int j = n - 1; j >= 0; j--)
            {
                btn[c, j].BackColor = Color.DodgerBlue;
                Refresh();
                Thread.Sleep(5);
                a[c, j] = 2;
            }
        ...
        private void draw_stol(Button[,] btn, int[,] a, int c)
        {
            for (int i = n - 1; i >= 0; i--)
            {
                btn[i, c].BackColor = Color.Yellow;
                Refresh();
                Thread.Sleep(5);
                a[i, c] = 1;
            }
        ...
        private void Level_10_Load(object sender, EventArgs e)
        {
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
        ...
        private void Level_10_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            if (ch == 6 && ch1 == 19 && a[1, 0] == 1 && a[1, 1] == 1
                && a[1, 2] == 1 && a[3, 0] == 1 && a[3, 1] == 1 &&
                a[3, 2] == 1)
            {
